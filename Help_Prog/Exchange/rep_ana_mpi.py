...
for line in i:
	...
	#	print('number of temp: '+str(Tnum))
	elif line.startswith('Dimension of temperature values:'):
		flag, Tdim = line2.split(":")		
		Tdim = int(Tdim)
	elif line.startswith('Number of lambda values:'):
		flag, Lnum = line2.split(":")
		Lnum = int(Lnum)
	elif line.startswith('Number of coupled lambda and temperature values:'):
		flag, Lnum = line2.split(":")
		Lnum = int(Lnum)
		Tnum = 1
		...
		if tflag == 0:
			try:
				ID, part, run, L1, T1, E1, L2, T2, E2, prob, s, s1, s2 = line2.split()
				ID = int(ID)
				part = int(part)
				run = int(run)
				s = int(s)
				step = [ID, part, run, L1, T1, E1, L2, T2, E2, prob, s, s1, s2]
				STEPS.append(step)
			#	print str(step)
			except ValueError:
